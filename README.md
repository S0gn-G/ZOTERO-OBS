...
3. 打开“设置”：
   ...
   - “研究领域 / 写作偏好”可留空。
4. 保存设置并刷新 Zotero 文献。
5. 搜索或筛选文献，单篇生成，或勾选当前列表后批量生成；Zotero 来源变化后可在“待同步”中一键更新当前结果。
...
| 全部 | 包括没有 PDF 的条目 |
| 未生成 | 尚无对应笔记的条目 |
| 待同步 | Zotero 条目或当前 PDF 附件在上次成功生成后发生变化的已有笔记 |
| 需处理 | 待同步、需原文、仅摘要、需修复或上次生成失败的条目 |

“全选当前列表”只选择当前搜索与筛选结果。“待同步”视图无需勾选即可用底部主按钮更新当前显示结果，也可以只勾选其中一部分。论文行中的“Zotero”会定位到原条目，“Obsidian”会打开按 `zotero_key` 实际找到的笔记，“PDF”使用系统默认阅读器打开附件。批量生成最多并发处理 3 篇文献。
...
| 未生成 | 尚无对应笔记 |
| 已生成 | 笔记完整 |
| 待同步 | Zotero 元数据或当前 PDF 附件自上次成功生成后发生变化 |
...
每次成功生成会在笔记 frontmatter 中写入内部字段 `zotero_source`，记录父条目修改时间、当前 PDF 附件身份及其修改时间。刷新时仅比较这一来源基线；本地手写导致的文件时间变化不会误触发同步。历史笔记缺少该字段时会进入“待同步”一次，未生成的论文不会被同步按钮隐式生成。

重新生成时，Markdown 和图片通过暂存、切换与回滚作为一个整体提交，避免新笔记与旧图片混用。任何图片缺失或提交失败都会撤销本次写入，`zotero_source` 也只在完整提交成功后推进。
...
刷新在工作线程中分页读取 Zotero，同时递归扫描输出目录中的 Markdown；搜索和五种视图仅在本地改变显示。来源基线只做等值比较，不解析时间或依赖本地文件 mtime。自动发现同样在工作线程执行，但只由“自动接入”按钮触发，结果经 UI 队列回到主线程；多 Vault 的选择和配置保存都发生在用户确认之后。设置、刷新、发现与生成不会同时执行状态性操作。
